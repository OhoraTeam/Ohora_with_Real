...
<link rel="stylesheet" href="../resources/cdn-main/notusercart.css">
									...
									<c:forEach var="item" items="${cartItems}">
										<div class="prdInfo xans-record-">
											...
											<input type="checkbox" id="basket_chk_id_${item.pdt_id}"
												...
												<div data-rate="${item.pdt_discount_rate}"
													data-price="${item.pdt_amount-((item.pdt_amount/100) * item.pdt_discount_rate)}"
													data-quantity="${item.pdt_count}"
													class="otherCheck checked" id="otherCheck_${item.pdt_id}">
												</div>
											</label> &nbsp;
                                     ...
                                     <label for="basket_chk_id_0" class="label_for_check">
                                      ...
                                      &nbsp; --%>
											...
											<!-- // 설명 -->
											<!-- 버튼 영역 -->
											<a href="<%= contextPath %>/cart/nodelbtn.do?pdt_id=${item.pdt_id}" class="btnNormal SMScart_option_del_btnTD delBtn">삭제</a>
						...
						<div
							...
							<div class="toggleArea type1 eToggle ">
								<div class="total title item">
									...
									<!-- 총 상품 할인 금액 -->
									<p class="discountAll">
										- <span class="discount"> <strongclass="prdDiscountAll">
...
function createOrderUrl() {
	...
	document.cookie = "basketItems=;max-age=0";
    ...
    checkedBoxes.forEach(checkbox => {
        ...
        // priceDiv가 제대로 선택되었는지 확인
        if (priceDiv) {
            const pdtId = priceDiv.id.split('_')[1]; // 상품 ID 추출
            const quantity = priceDiv.getAttribute('data-quantity'); // 수량 추출

            if (pdtId && quantity) { // 값이 존재하는지 확인
                urlParams.push(`pdtId=\${pdtId}`);
                urlParams.push(`pdtCount=\${quantity}`);
            }
        }
    });
    ...
    const baseUrl = `\${contextPath}/product/orderpage.do`; // contextPath를 포함한 기본 URL
    const queryString = urlParams.join('&');
    const fullUrl = `\${baseUrl}?\${queryString}`;
    
    alert('생성된 URL: ' + fullUrl); // 생성된 URL 확인을 위한 alert
    return fullUrl;
}
</script>
<script>
	document.addEventListener('DOMContentLoaded', function() {
	   ...
	   function calculateItemTotal(item) {
	       ...
	       // 개별 상품의 합계 표시 업데이트
	       const prdTotalElement = item.closest('.prdInfo').querySelector('.prdTotal strong');
	       if (prdTotalElement) {
	           prdTotalElement.textContent = itemTotal.toLocaleString();
	       }
	   ...
	   function calculateTotalPrice() {
	       ...
	       individualChecks.forEach(checkbox => {
	           if (checkbox.checked) {
	               const priceDiv = checkbox.nextElementSibling.querySelector('.otherCheck');
	               const itemTotal = calculateItemTotal(priceDiv);
	               total += itemTotal;
	           }
	       });
	       ...
	       // 금액 표시 업데이트
	       totalProductPrice.textContent = total.toLocaleString();
	       
	       // 최종 금액 계산 (상품금액 + 배송비)
	       const final = total + shipping;
	       shippingFee.textContent = shipping.toLocaleString();
	       finalPrice.textContent = final.toLocaleString();

	       // 무료배송까지 남은 금액 표시
	       const remainingForFree = Math.max(50000 - total, 0);
	       if (remainingForFree > 0) {
	           document.querySelector('.amount').textContent = remainingForFree.toLocaleString();
	       } else {
	           document.querySelector('.total_info_txt.delv').innerHTML = '<b>무료배송</b>';
	       }
	   }
...
function createOrderUrl() {
	console.log("현재 쿠키:", document.cookie);
	  document.cookie = "basketItems=; path=/; domain=" + window.location.hostname + "; expires=Thu, 01 Jan 1970 00:00:00 GMT";
    const checkedBoxes = document.querySelectorAll('.individual-checkbox:checked');
    let urlParams = [];
